...
export function SalaryManagement({ userId, isAdmin, isManager }) {
  ...
  const generateMonthlySalaries = async () => {
    ...
    try {
      ...
      toast({ title: "Success", description: "Salaries generated successfully", variant: "default" });
      fetchData();
    } catch (error) {
      console.error("Error generating salaries:", error);
      toast({ title: "Error", description: "Failed to generate salaries", variant: "destructive" });
    } finally {
      setGenerating(false);
    }
  };
  ...
  const openEditDialog = (salary) => {
    ...
    setFormData({
      ...
      net_salary_manual: salary.net_salary_manual || 0,
      manager_justification: salary.manager_justification || "",
    });
    setEditDialogOpen(true);
  };

  const openHistoryDialog = async (salary) => {
    setSelectedSalary(salary);
    try {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: "Success", description: "Salary updated successfully", variant: "default" });
  ...
  const handleApprove = async (salaryId) => {
    try {
      ...
      toast({ title: "Success", description: "Salary approved", variant: "default" });
      fetchData();
    } catch (error) {
      console.error("Error approving salary:", error);
      toast({ title: "Error", description: "Failed to approve salary", variant: "destructive" });
    }
  };

  const handleLock = async (salaryId) => {
    try {
      const { error } = await supabase
        .from("salaries")
        .update({
      ...
      toast({ title: "Success", description: "Salary locked", variant: "default" });
      fetchData();
    } catch (error) {
      console.error("Error locking salary:", error);
      toast({ title: "Error", description: "Failed to lock salary", variant: "destructive" });
    }
  };
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      toast({ title: "Success", description: "Salary unlocked", variant: "default" });
      setUnlockDialogOpen(false);
      fetchData();
    } catch (error) {
      console.error("Error unlocking salary:", error);
      toast({ title: "Error", description: "Failed to unlock salary", variant: "destructive" });
    }
  };
  ...
  const exportToCSV = () => {
    ...
    toast({ title: "Success", description: "CSV exported", variant: "default" });
  };
  ...
  const handleBulkApprove = async () => {
    ...
    if (toApprove.length === 0) {
      toast({ title: "Info", description: "No salaries to approve", variant: "default" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Success", description: `${toApprove.length} salaries approved`, variant: "default" });
      fetchData();
    } catch (error) {
      console.error("Error bulk approving salaries:", error);
      toast({ title: "Error", description: "Failed to approve salaries", variant: "destructive" });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleLockAll = async () => {
    ...
    if (toLock.length === 0) {
      toast({ title: "Info", description: "No salaries to lock", variant: "default" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Success", description: `${toLock.length} salaries locked`, variant: "default" });
      fetchData();
    } catch (error) {
      console.error("Error locking salaries:", error);
      toast({ title: "Error", description: "Failed to lock salaries", variant: "destructive" });
    } finally {
      setIsSubmitting(false);
    }
  };
            ...
            <div className="p-4 border rounded-lg bg-primary/5">
              ...
              <div className="flex items-center gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData(p => ({ ...p, net_salary_manual: Number(e.target.value) }))}
                />
              </div>
            </div>
            ...
            {formData.net_salary_manual && (
              <div className="p-3 bg-blue-50 dark:bg-blue-900 rounded border border-blue-200 dark:border-blue-700">
                <p className="text-sm font-medium">
                  Override Amount: <span className="text-lg text-blue-600 dark:text-blue-300">₹{formData.net_salary_manual.toLocaleString()}</span>
                </p>
              </div>
            )}
          </div>
                  ...
                  <div key={record.id} className="border rounded-lg p-3">
                    ...
                    {record.new_data && typeof record.new_data === 'object' && (
                      <div className="mt-2 text-xs">
                        ...
                        <span>₹{(record.new_data?.final_salary || 0).toLocaleString()}</span>
                      </div>
                    )}
                  </div>
                ))}
